...
interface ISingleTestimonialPanel {
	cTAButton: URLField
	testimonials: ContentItem<ITestimonial>[]
}

const SingleTestimonialPanel = async ({ module, languageCode }: UnloadedModuleProps) => {
	const {
		fields: { cTAButton, testimonials },
		contentID
	} = await getContentItem<ISingleTestimonialPanel>({
				...
				<div className="relative mx-auto max-w-2xl py-24 sm:py-32 lg:max-w-4xl">
					...
					<figure className="grid grid-cols-1 items-center gap-x-6 gap-y-8 lg:gap-x-10">
						...
						<div className="col-end-1 w-16 lg:row-span-4 lg:w-72">
							{testimonial.fields.headshot && (
								<AgilityPic
									image={testimonial.fields.headshot}
									className="rounded-xl bg-purple-50 lg:rounded-3xl"
									fallbackWidth={200}
									alt={testimonial.fields.headshot.label || testimonial.fields.title || ""}
									sources={[{ media: "(min-width: 768px)", width: 600 }]}
								/>
							)}
						</div>
								...
								<div>
									{testimonial.fields.companyLogo && (
										<AgilityPic
											...
											fallbackWidth={100}
											alt={
												testimonial.fields.companyLogo.label ||
